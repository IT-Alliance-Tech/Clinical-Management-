...
const INITIAL_FORM_STATE = {
  ...
  experience: "",
  consultationFee: "",
  availability: { days: [], startTime: "09:00", endTime: "17:00", slotDuration: 30 },
  isActive: true,
};
...
const FilterSection = memo(({ filters, setFilters, doctorsCount, filteredCount, onClear }) => (
  <Card sx={{ mb: 3, border: "1px solid #e0e0e0" }}>
    <CardContent sx={{ p: 3 }}>
      <Box sx={{ display: "flex", alignItems: "center", mb: 3 }}>
        ...
        <Button
          ...
          sx={{
            borderColor: "#2e7d32",
            color: "#2e7d32",
            "&:hover": { borderColor: "#1b5e20", backgroundColor: "#f1f8f4" },
          }}
        >
          Reset Filters
        </Button>
      ...
      <Typography variant="body2" color="text.secondary" sx={{ mt: 2 }}>
        Showing {filteredCount} of {doctorsCount} doctors
      </Typography>
    </CardContent>
  </Card>
));
...
/* ---------------- DOCTOR TABLE ROW COMPONENT ---------------- */
const DoctorTableRow = memo(({ doctor, onEdit, onToggleStatus, onDelete }) => (
  <TableRow sx={{ "&:hover": { backgroundColor: "#f9fafb" } }}>
    <TableCell sx={{ fontWeight: 500 }}>{doctor.name}</TableCell>
    ...
    <TableCell sx={{ fontWeight: 500 }}>₹{doctor.consultationFee}</TableCell>
    <TableCell>
      <Box sx={{ display: "flex", gap: 0.5, flexWrap: "wrap" }}>
        {(doctor.availability?.days || []).map((day) => (
          <Chip
            ...
            label={day.slice(0, 3)}
            size="small"
            sx={{ backgroundColor: "#f0f0f0", fontSize: "0.7rem" }}
          />
        ))}
      </Box>
    </TableCell>
    ...
    <TableCell>
      <Chip
        ...
        size="small"
        sx={{
          backgroundColor: doctor.isActive ? "#2e7d32" : "#9e9e9e",
          color: "#fff",
          fontWeight: 600,
          minWidth: 70,
        }}
    ...
    <TableCell align="center">
      <Box sx={{ display: "flex", gap: 0.5, justifyContent: "center", flexWrap: "wrap" }}>
        <Button
          ...
          sx={{ minWidth: "auto", p: 1, color: "#2e7d32", "&:hover": { backgroundColor: "#e8f5e9" } }}
        >
        ...
        <Button
          ...
          sx={{
            minWidth: "auto",
            p: 1,
            color: doctor.isActive ? "#ed6c02" : "#2e7d32",
            "&:hover": { backgroundColor: doctor.isActive ? "#fff3e0" : "#e8f5e9" },
          }}
        >
          {doctor.isActive ? <ToggleOffIcon fontSize="small" /> : <ToggleOnIcon fontSize="small" />}
        </Button>
        <Button
          size="small"
          onClick={() => onDelete(doctor._id)}
          sx={{ minWidth: "auto", p: 1, color: "#d32f2f", "&:hover": { backgroundColor: "#ffebee" } }}
        >
...
DoctorTableRow.displayName = "DoctorTableRow";

/* ---------------- DOCTOR DIALOG COMPONENT ---------------- */
const DoctorDialog = memo(({ open, onClose, form, setForm, onSubmit, editingDoctor, uploading }) => {
  ...
  return (
    <Dialog open={open} onClose={onClose} maxWidth="md" fullWidth PaperProps={{ sx: { borderRadius: 3 } }}>
      ...
      <DialogContent dividers sx={{ py: 3 }}>
          ...
          <Grid item xs={12} sm={4}>
            <Box
              ...
              sx={{
                ...
                cursor: "pointer",
                "&:hover": { borderColor: "#2e7d32", backgroundColor: "#f1f8f4" },
              }}
            >
              <Typography
                variant="body2"
              ...
              <Typography sx={{ color: "#2e7d32", fontWeight: 600 }}>Browse</Typography>
              <input
                type="file"
                hidden
                accept="image/*"
        ...
        <Typography sx={{ mt: 3, mb: 1, fontWeight: 600 }}>Available Days</Typography>
        <Box sx={{ display: "flex", gap: 1, flexWrap: "wrap", mb: 3 }}>
          {DAYS.map((d) => (
            <Chip
              ...
              sx={{
                minWidth: 90,
                backgroundColor: form.availability.days.includes(d) ? "#2e7d32" : "#f0f0f0",
...
DoctorDialog.displayName = "DoctorDialog";
...
export default function DoctorsAdminPage() {
  ...
  // Upload image
  const uploadDoctorImage = useCallback(async (file) => {
    if (!file) return null;
    setUploading(true);
    ...
    const { error } = await supabase.storage.from("doctor-images").upload(fileName, file);
  ...
  const openEditDialog = useCallback((doc) => {
  ...
  // Submit doctor
  const submitDoctor = useCallback(async (doctorImage) => {
    ...
    const endpoint = editingDoctor ? `/api/doctors/${editingDoctor._id}` : `/api/doctors`;
    ...
    setOpenDialog(false);
    fetchDoctors();
  }, [form, editingDoctor, uploadDoctorImage, fetchDoctors]);

  // Toggle status
  const toggleStatus = useCallback(async (doc) => {
  ...
  const filteredDoctors = useMemo(() => {
    ...
    return doctors.filter((doc) => (
      ...
      (filters.availableDay === "" || doc.availability?.days?.includes(filters.availableDay))
    ));
  }, [doctors, filters]);
            ...
            <Box sx={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 2 }}>
              ...
              <Box sx={{ display: "flex", gap: 2, alignItems: "center" }}>
                <Chip
                  ...
                  sx={{ backgroundColor: "#2e7d32", color: "#fff", fontWeight: 600, px: 2, py: 2.5 }}
                />
                <Button
                  variant="contained"
                  startIcon={<AddIcon />}
